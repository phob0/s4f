...
export const LoginModalButton: FC<LoginModalButtonProps> = memo(({
  ...
  const {
    ...
    setLoggingInState,
  } = useLogin({ token: process.env.LOGIN_TOKEN });

  const { expires, loginToken, signature } = useLoginInfo();

  console.log(expires, loginToken, signature, process.env.NEXT_PUBLIC_LOGIN_TOKEN)
